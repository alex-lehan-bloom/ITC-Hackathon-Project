...
number_of_places=5
infos_need = ['id','name','address','rating','coordinates']
# input places info and requirements
with open('../back-end/sample_data_set.json') as json_file:
    datas_pos = json.load(json_file)
with open('../back-end/requirements.json') as json_file:
    requirements = json.load(json_file)
# create visitors num table with uid
pos_poptimes = []
...
# export to json
with open('respdata.json', 'w') as fp:
    json.dump(respdata.to_json(), fp)
